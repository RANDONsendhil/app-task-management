...
window.applyProjectFilters = function () {
	...
	projectRows.forEach((row) => {
		...
		const projectDescription = (
			row.getAttribute("data-project-description") || ""
		).toLowerCase();
		...
		// Status filter
		const statusMatch =
			!statusFilterValue || projectStatus === statusFilterValue;
...
window.updateProjectFilterStats = function (visibleCount, totalCount) {
	...
	if (statsElement) {
		if (visibleCount === totalCount) {
			statsElement.textContent = `${totalCount} projet${
				totalCount !== 1 ? "s" : ""
			}`;
			statsElement.style.color = "#6c757d";
		} else {
			statsElement.textContent = `${visibleCount} sur ${totalCount} projet${
				totalCount !== 1 ? "s" : ""
			}`;
...
document.addEventListener("keydown", function (event) {
	if (event.key === "Escape") {
		const projectCreationModal = document.getElementById(
			"projectCreationModal"
		);
		if (
			projectCreationModal &&
			projectCreationModal.style.display === "block"
		) {
...
function editProject(projectId) {
	...
	document.getElementById("edit_project_nom").value = row
		.querySelector(".project-name-cell")
		.innerText.trim();
	document.getElementById("edit_project_description").value = row
		.querySelector(".project-description-cell")
		.innerText.trim();
	document.getElementById("edit_project_date_debut").value = row
		.querySelectorAll(".project-date-cell")[0]
		.innerText.trim();
	document.getElementById("edit_project_date_fin").value = row
		.querySelectorAll(".project-date-cell")[1]
		.innerText.trim();
